...
import { FillFormModel } from "../models/forms.model";
...
const handleResponse = async (ctx: any, type: string) => {
  ...
  try {
    ...
    if(type !== 'message') {
      ...
      if(action === 'confirm') {
        ...
        const _input = await formBridge.telegram(_chatSession.formId, _chatSession.nextInput);
        if (!_input) {
        ...
        if(!_input.telegram){
          ...
          if (nextInputId) {
            const nextInput = await formBridge.telegram(_chatSession.formId, nextInputId);
            if (!nextInput) {
              finalizeForm(_chatSession.replies, _chatSession.formId, userId)
              return;
            }
            ...
            telegramInstance.sendMessage(userId, ...nextInput.telegram).then((res) => {
              ...
              chat.set(chatId, {formId, inputId: nextInput._id, nextInput: nextInput.nextInput, replies, prevMessageId: messageId, inputIds: [...session.inputIds, nextInput._id]})
            })
          }
        ...
        telegramInstance.sendMessage(userId, ..._input.telegram).then((res) => {
          ...
          chat.set(chatId, {formId, inputId: _input._id, nextInput: _input.nextInput, replies, prevMessageId: messageId, inputIds: [...session.inputIds, _input._id]})
        })
        return
      }else if(action === 'edit' && !confIinputId) { // for editing the whole form
        ...
        const buttons = [...replies.values()].map((reply, index) => {
          return [{text: reply.question, callback_data: `${confFormId}__edit__${inputIds[index]}`}]
        })
        telegramInstance.sendMessage(userId, 'Which input do you want to edit?', {
          reply_markup: {
            inline_keyboard: buttons
          }
        })
        return
      }else if(action === 'edit') { // for editing an input
        const _chatSession = chat.get(chatId)
        if(!_chatSession) {
        ...
        const _input = await formBridge.telegram(_chatSession.formId, confIinputId);
        if (!_input) {
          telegramInstance.sendMessage(userId, 'This form has no inputs');
          return;
        }
        ...
        const buttons = [{text: 'Yes', callback_data: `${confFormId}__cancel.confirm`}, {text: 'No', callback_data: `${confFormId}__cancel.cancel`}].map((button) => {
          return [button]
        })
        telegramInstance.sendMessage(userId, 'Are you sure you want to cancel filling this form?', {
          reply_markup: {
            inline_keyboard: buttons
          }
        })
        ...
        chat.delete(chatId)
        telegramInstance.sendMessage(userId, 'You have cancelled filling this form')
        return
      }
  ...
  if(!chatSession) {
    ...
    chat.set(chatId, {formId, inputId: '0', nextInput: input.nextInput, replies, prevMessageId: messageId, inputIds: ['0']})
    })
  }else {
    telegramInstance.sendMessage(userId, 'You are not filling a form')
  }
  ...
  if(type === 'message') {
    ...
    chat.set(chatId, {...chatSession, ...(chatSession.completed ? {nextInput: null} : {}), replies})
    const buttons = [{text: 'Yes', callback_data: `${formId}__confirm__${inputId}`}, {text: 'No, Edit it', callback_data: `${formId}__edit__${inputId}`}].map((button) => {
      return [button]
    })
    telegramInstance.sendMessage(userId, formBridge.escapeMarkdown(`Question: *${question}* \n\nYour answer: *${response}* \n\nIs this correct?`), {
  ...
  // check if the user is filling a form
  if(!formId || !inputId) {
    ctx.from && telegramInstance.sendMessage(userId, 'You are not filling a form')
    return
  }
  // check if the user is filling the right form
  if(formId !== chatSession.formId) {
    ctx.from && telegramInstance.sendMessage(userId, 'You are not filling this form')
    return
  }
  // check if the user is filling the right input
  if(inputId !== chatSession.inputId) {
    ctx.from && telegramInstance.sendMessage(userId, 'You are not filling this input')
    return
  }
  ...
  if (!input) {
    ctx.from && telegramInstance.sendMessage(userId, 'This form has no inputs');
    return;
  }
  ...
  telegramInstance.sendMessage(userId, ...input.telegram).then((res) => {
    ...
    chat.set(chatId, {formId, inputId: nextInput, nextInput: input.nextInput, replies, prevMessageId: messageId, inputIds: [...chatSession.inputIds, input._id]})
  })
  
  } catch (error) {
    telegramInstance.sendMessage(userId, 'An error occured')
  }
}
...
telegramInstance.on('message', async (ctx) => {
  ...
  if (text?.startsWith('/start') && !chat.get(ctx.chat?.id)) {
    ...
    if(!formId || formId.length !== 24 || !formId.match(/^[0-9a-fA-F]{24}$/)) {
      //@ts-ignore
      telegramInstance.sendMessage(ctx?.from?.id, 'Please provide a valid form id./n/nThe command should be in this format: /cancel/5f9b3b3b3b3b3b3b3b3b3b3b')
      return
    }
    // send an inline keyboard asking if the user really wants to start fillling
    // the form
    ...
    telegramInstance.sendMessage(ctx?.from?.id || 6554560778, `Do you want to cancel filling this form with id ${formId}?`, {
      reply_markup: {
        inline_keyboard: buttons
      }
    })
...
let message = 'Please confirm your answers below:'
  ...
  const buttons = [{text: 'Submit', callback_data: `${formId}__submit`}, {text: 'Edit', callback_data: `${formId}__edit`}, {text: 'Cancel', callback_data: `${formId}__cancel`}].map((button) => {
    return [button]
  })
...
const saveForm = async (replies: any, formId: String, userId: string) => {
  ...
  const saved = await FillFormModel.create({formId, data: {...data, ___chanel: 'telegram', ___userId: userId}})
  if(!saved) {
    telegramInstance.sendMessage(userId, 'An error occured while saving your response. Please try again')
    return
  }
  telegramInstance.sendMessage(userId, 'Thank you for your response. Your response has been saved.')
  chat.delete(userId)
}
